...
public:
    ...
    double* getMins()
    {
        return _network->mins;
    }
        
    double* getMaxes()
    {
        return _network->maxes;
    }
    
    double** getWeights(int sourceLayer){
	return _network->matrix[sourceLayer][0];
    }

    double** getBias(int sourceLayer){
	return _network->matrix[sourceLayer][1];
    }
    int getLayerSize(int sourceLayer){
	return _network->layerSizes[sourceLayer];
    }
    int getLayerNum(){
	return _network-> numLayers;
    } 
    int get_inputdim(){
	std::cout<<"Enter Acas get_inputdim"<<std::endl;
	return _network->layerSizes[0];
    }

...
bool inBound(pair<double, double> bound, double x) {
    return FloatUtils::lte(bound.first,x) && FloatUtils::lte(x,bound.second);
}
 ...
 * @return
 */
unsigned index2order(AcasNeuralNetwork nnet, unsigned Layer, unsigned node) {
    unsigned  res = 0;
    for (int i = 0; i < Layer; ++i) {
        res += nnet.getLayerSize(i);
    }
    return res + node;
}
...
//    if (argc < 3)
//        AIOff = false;
//    else if ("-AIOff" == string(argv[2]))
//        AIOff = true;
    //    cout << argv[2] << endl;
    cout << "AIOff = " <<AIOff <<endl;
...
//    if (!AIOff) {
        ...
        obounds<<"";
        obounds.close();
        //std::string path("/home/jiangchao/AIProjects/reluplex/ReluplexCav2017/nnet/ACASXU_run2a_3_6_batch_2000.nnet");
        ...
        amatrix.set_lowerbound(0,normalizeInput(0,55947.691,neuralNetwork));
        amatrix.set_lowerbound(3,normalizeInput(3,1145,neuralNetwork));
        amatrix.set_uperbound(4,normalizeInput(4,60,neuralNetwork));

        // amatrix.set_lowerbound(0,0.6);
        // amatrix.set_lowerbound(3,0.3);
        // amatrix.set_uperbound(4,0.5);
        ...
        AbsEle *aele = new AbsEle(inputDim,name,TOPO,num,coffs,lop,bias);
...
//=============== ReLu ================
    ...
    ifstream *ibounds;
    if (!AIOff) {
        ibounds =  new ifstream("bounds.txt",std::ios::in);
    }
    ...
    vector<pair<double,double>> bounds_vec;
    ...
    for ( unsigned i = 1; i < numLayersInUse - 1; ++i )
    {
        for ( unsigned j = 0; j < neuralNetwork.getLayerSize( i ); ++j )
        {
            ...
            if (!AIOff){
                double inf,sup;
                (*ibounds) >> inf >> sup;
                bounds_vec.push_back(make_pair(inf,sup));
//                if(i==1){
                    reluplex.setUpperBound(f,std::max(sup,0.0));
                    reluplex.setLowerBound(f,std::max(inf,0.0));
...
//                }
                cout << "ABS,\t Layer \t" << i-1 << ",\tNode \t" << j <<",\t"
                     ...
                     <<inf << ",\t" << sup ;
                if ( ( FloatUtils::isPositive(inf) && FloatUtils::isPositive(sup) ) ||
                     ( FloatUtils::isNegative(inf) && FloatUtils::isNegative(sup) ) ) {
//                    reluplex.setLowerBound(b,inf);
//                    reluplex.setUpperBound(b,sup);
//                    if ( FloatUtils::isPositive(inf) && FloatUtils::isPositive(sup)) {
//                        reluplex.setLowerBound(b,0.0);
//                    }
//                    else {
//                        reluplex.setUpperBound(b,0.0);
//                    }
                    cout << ",\t***";
                }
                cout << endl;
//                reluplex.setLowerBound( f, 0.0 );
    ...
    if (!AIOff){
        for ( unsigned j = 0; j < outputLayerSize; ++j ){
            unsigned v = nodeToVars[Index(numLayersInUse, j, false)];
            double inf,sup;
            (*ibounds) >> inf >> sup;
            reluplex.setLowerBound(v,inf);
            reluplex.setUpperBound(v,sup);
            cout << "ABS,\t Layer \t" << numLayersInUse-1 << ",\tNode \t" << j <<",\t"
                 << v << ",\t" << -1 <<",\t"
                 //<< std::scientific
                 << std::setprecision (std::numeric_limits<double>::digits10 + 1)
                 <<inf << ",\t" << sup ;

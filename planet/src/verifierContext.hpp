...
//===== LI JIANLIN =====
    ...
    boost::posix_time::time_duration initialWorkTime;
    boost::posix_time::time_duration solveTime;

    const std::vector<std::string>& getNodeNames() const {
        return this->nodeNames;
    }
    const std::vector<std::pair<double,double>>&  getInitialNeuronLimitBounds() const {
        return this->initialNeuronLimitBounds;
    };
    const std::map<std::string,unsigned int>& getNodeNumberLookup(){
        return this->nodeNumberLookup;
    };
    bool computeInitialNeuronLimitBounds_imp();
